...
const LOCAL_MODEL_URL = "/models/mediapipe_face_detector_short/model.json";
const TFHUB_MODEL_URL =
...
async function createFaceDetector(detectorModelUrl: string) {
  await tf.ready();
...
export function loadFaceDetector() {
  if (!detectorPromise) {
    detectorPromise = createFaceDetector(LOCAL_MODEL_URL).catch((error: unknown) => {
      console.warn(
        "[FaceDetector] Local TFJS detector failed; falling back to TFHub.",
        error,
      );
      return createFaceDetector(TFHUB_MODEL_URL);
    });
  }
