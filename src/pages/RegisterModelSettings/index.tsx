...
export default function RegisterModelSettings() {
  const settingsMode = "register";
  ...
  const modelGroups: Array<{ key: ModelGroup; title: string; description: string; icon: typeof faServer }> = [
    { key: "backend_models", title: "Backend Models", description: "Server-side settings from ai/config/register_model_setting.json", icon: faServer },
    { key: "frontend_models", title: "Frontend Models", description: "Browser-side settings from ai/config/register_model_setting.json", icon: faWindowMaximize },
  ];
  ...
  function renderSettings(model: ModelSchema, group: ModelGroup, settingKeys?: readonly string[]) {
    ...
    const settings = settingKeys
      ...
      : Object.entries(model.settings_values).filter(([key]) => {
          ...
          if (key.startsWith("register_")) return true;
          if (key.startsWith("verify_")) return false;
  ...
  return (
    <section className={styles.page} aria-labelledby="models-title">
      <div className={styles.heading}>
        <div>
          <h2 id="models-title">ตั้งค่าโมเดลลงทะเบียน</h2>
          <p>ค่าที่ใช้ตอนลงทะเบียนใบหน้า (เข้มงวดกว่า)</p>
        </div>
      </div>
      {error ? (
            ...
            <div className={styles.models}>
              ...
              {(document?.model_schemas[modelGroup.key] ?? [])
                ...
                <article className={styles.model} key={model.model_key}>
                  ...
                  {model.model_key === "face_landmarker" ? renderFaceLandmarkerWorkbench(model) : renderSettings(model, modelGroup.key)}
